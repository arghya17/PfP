...
void print256i_u32(__m256i var, int start)
{
  ...
  cout << "Values [" << start << ":" << start + 8 << "]: " << val[0] << " "
       << val[1] << " " << val[2] << " " << val[3] << " "
       << val[4] << " "
       << val[5] << " " << val[6] << " " << val[7] << "\n";
}
...
int sse4_version(const int *__restrict__ source, int *__restrict__ dest)
{
  ...
  for (int i = 0; i < N; i += stride)
  {
    ...
    // formula (z << 6) | (y << 4) | (x << 2) | w._mm_slli_si128
...
int avx2_version(const int *__restrict__ source, int *__restrict__ dest)
{
  ...
  const int stride = 256 / (sizeof(int) * CHAR_BIT);
  ...
  for (int i = 0; i < N; i += stride)
  {
    ...
    // loading the 8 32 bit integers [a,b,c,d,e,f,g,h]
    __m256i mask = _mm256_setr_epi32(0x00000000, -1, -1, -1, -1, -1, -1, -1);
    // shifting to get [e,f,g,h,0,0,0,0] in x_shift
    __m256i tmp0 = _mm256_and_si256(shifted, mask);
    // shifting left by 4 bytes to get [b,c,d,e,f,g,h,0]
    __m256i tmp1 = _mm256_add_epi32(x, tmp0);
    // adding to get [a+b,b+c,c+d,d+e,e+f,f+g,g+h,h] in tmp1
    shifted = _mm256_permutevar8x32_epi32(tmp1, _mm256_setr_epi32(6, 7, 0, 1, 2, 3, 4, 5));
    mask = _mm256_setr_epi32(0x00000000, 0x00000000, -1, -1, -1, -1, -1, -1);
    __m256i tmp2 = _mm256_and_si256(shifted, mask);
    // shifting left by 8 bytes to get [c+d,d+e,e+f,f+g,g+h,h,0,0]
    __m256i tmp3 = _mm256_add_epi32(tmp2, tmp1);
    // adding to get [a+b+c+d,b+c+d+e,c+d+e+f,d+e+f+g,e+f+g+h,f+g+h,g+h,h] in tmp3
    shifted = _mm256_permutevar8x32_epi32(tmp3, _mm256_setr_epi32(0, 0, 0, 0, 0, 1, 2, 3));
    mask = _mm256_setr_epi32(0x00000000, 0x00000000, 0x00000000, 0x00000000, -1, -1, -1, -1);
    __m256i tmp4 = _mm256_and_si256(shifted, mask);
    // shifting left by 16 bytes to get [e+f+g+h,f+g+h,g+h,h,0,0,0,0]
    __m256i out = _mm256_add_epi32(tmp4, tmp3);

    // adding to get [a+b+c+d+e+f+g+h,b+c+d+e+f+g+h,c+d+e+f+g+h,d+e+f+g+h,e+f+g+h,f+g+h,g+h,h] in out
