...
struct word_tracker
{
    // fixed the number of threads to 5
    // earlier word_count was of size 4 which was causing race condiotion and accessing total_lines_processed variable
    //  This was leading to wrong output for total lines processed
    alignas(64) PaddedCounter word_count[5];
    //others may aslo give false sharing misses like total_lines_processed and total_words_processed should not give false sharing misses
    alignas(64) uint64_t total_lines_processed;
    alignas(64) uint64_t total_words_processed;
    alignas(64) pthread_mutex_t word_count_mutex;
} tracker;
...
void *thread_runner(void *th_args)
{
    ...
    while (getline(input_file, line))
    {
        ...
        while ((pos = line.find(delimiter)) != std::string::npos)
        {
            ...
            tracker.word_count[thread_id].count++;
            // true sharing: on lock and total word variable
            // reduce true sharing by recording local count and updating global count at once
            line.erase(0, pos + delimiter.length());
        }
    }
    // reduce true sharing by recording local count and updating global count at once
    pthread_mutex_lock(&tracker.word_count_mutex);
    tracker.total_words_processed += tracker.word_count[thread_id].count;
    pthread_mutex_unlock(&tracker.word_count_mutex);
    pthread_mutex_lock(&line_count_mutex);
    tracker.total_lines_processed += num_lines;
